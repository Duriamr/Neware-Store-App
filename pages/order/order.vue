<template>
	<view>
		<view class="order_content">
			...
			<view class="pay_ul" >
				<view class="pay_list" v-if='Number(sumPrice) < Number(paymaxamount)' @tap.stop="tabPayType(1)">
					<view class="left">
						<text>线上支付</text>
...
<script>
	export default {
		...
		onLoad(option) {
			...
			if(option.type=='again' || option.type=='purchase'){
				...
				uni.getStorage({
					...
					success: (ret) => {
						...
						for(let i=0; i<this.buylist.length; i++){
							this.goodsPrice = this.goodsPrice + (this.buylist[i].quantity*this.buylist[i].price);
						}
					...
					success: (res) => {
						if (res.confirm){
							...
							if(this.type == 'shoppingCart'){
								let cartIds = [];
								for(let i=0; i<this.buylist.length; i++){
									cartIds.push(this.buylist[i].id);
								}
								uni.request({
								    url: this.$url+'/api/v2/order/add', 
									header:{
										Authorization:'Bearer '+this.token
									},
									data: {
								...
								uni.request({
								    url: this.$url+'/api/v2/order/buyagain', 
									header:{
										Authorization:'Bearer '+this.token
									},
									data: {
										...
										"quantity": this.buylist.length,
										"payMent": 0,
										"deliveryAddressId": this.address.id,
										"platform": this.platform,
										"buyerRemark": this.remark,
										"customerNo": this.customerNo,
										"orderDetails": this.buylist,
										"isOnlinePay":isOnlinePay
								    },
									method: "POST",
								    success: (res) => {
								...
								let products = [];
								let len = this.buylist.length-1
								for(let i=0; i<len; i++){
									products.push(this.buylist[i]);
								}
								uni.request({
								    url: this.$url+'/api/order/addcampaignv2', 
									header:{
										Authorization:'Bearer '+this.token
									},
									data: {
										"campaignId": 2,
										"products":products,
										"giveProductId": this.buylist[len].giveProductId,
										"platform": this.platform,
										"payMent": 0,
										"deliveryAddressId": this.address.id,
										"buyerRemark": this.remark,

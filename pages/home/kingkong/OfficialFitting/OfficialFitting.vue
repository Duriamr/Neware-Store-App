					...
					<view class="grid_list" v-for="(item2, index2) in item.products" :key="index2" @tap="$toGoods(item2.productId)">
						...
						<view class='productDetaile'>
							...
							<view class='money'>
								...
								<text class="clickLook"  @tap="$toGoods(item2.productId)">点击查看</text>
							</view>
						</view>
					</view>
				</view>
	...
	export default {
		...
		methods:{
			getLoadData(){
				...
				uni.request({
				    url: this.$url+'/api/officialfitting/banner',  
					method: "GET",
				...
				uni.request({
				    url: this.$url+'/api/officialfitting/multi', 
					method: "GET",
